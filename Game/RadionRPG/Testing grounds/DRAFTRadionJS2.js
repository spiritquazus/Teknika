...
function Melee() {
    Hitchance();
    if (hitchance > 0) {
        monsterHealth - meleeAttackAttempt(melee[equipped].damageMax, melee[equipped].damageMin, monsters[fighting].level, meleeAdditiveCalc);
        ...
        monsterHealth - (meleeAttackAttempt(melee[equipped].damageMax, melee[equipped].damageMin, monsters[fighting].level, meleeAdditiveCalc))*0.3;
        text.innerText += "You missed the swing and only grazed the monster! Your damage was greatly reduced..."
...
function monsterTurn() {
    const mobhitchance = ((Math.floor(Math.random()) * 100 ));
    //maybe change the 85 hitchance below to a dynamic value?
    if (mobhitchance > 15) {
        health -= getMonsterAttackValue(monsters[fighting].damageMax, monsters[fighting].damageMin, armor);
    } else {
        health -= (Math.floor((getMonsterAttackValue(monsters[fighting].damageMax, monsters[fighting].damageMin, armor) * 0.75)));
        console.log("mob missed! Graze")
        text.innerText += "<br>The " + monsters[fighting].name + " fumbled and grazed you."
    }
}
...
//player death
    function gameOver() {
        ...
        text.innerText += "<br>.<br><br>.<br><br>.<br>You have died! The Zone claims another life..."
...
function getMonsterAttackValue(atkMax, atkMin, PCarmor) {
    const hitmob = (Math.floor(Math.random() * ((atkMax-atkMin)) + atkMin)) - (Math.floor(Math.random() * ((1 - 0.5) + 0.5) * PCarmor)); //basic atk formula. range of attack of monster - random armor multiplier down to half
    return hitmob > 0 ? hitmob : 0; //stop the armor-absorbed hits from "healing" lmao. les degats ne sont pas negatifs.
    console.log("Player ate dmg: ", hitmob);
}

//melee attack CALC 
function meleeAttackAttempt(meleeatkMax, meleeatkMin, moblvl, meleeadditives) {
    const hitmelee = (Math.floor(Math.random() * ((meleeatkMax-meleeatkMin)) + meleeatkMin + meleeadditives)) - (Math.floor(Math.random() * ((0.4 - 0.2) + 0.2) * moblvl)); //idem but monster lvl
    return hitmelee > 0 ? hitmelee : 0; //boom cest le choc
...
function Hitchance(hitvalue) { 
    const hitchance = (Math.floor(Math.random() * (100-hitvalue) + hitvalue )) - (Math.floor(Math.random()*100));
    return hitchance > 0 ? hitchance: 0;
    console.log("Hit yes or no", hitchance) 
}
...
function shootingAttack(rangedatkMax, rangedatkMin, moblvl, rangedadditives, rangedpenetration) {
    ...
    return hitshooty > 0 ? hitshooty : 0; //boom cest le choc
    if (hitshooty === 0) {
        text.innerHTML += "<br> Oh dear, your attack didn't do much!";
    }
    console.log("Player slashd the miscreant! ", hitshooty, " dmg") ;
}
...
const meleeadditivesinternal = equippedart + equippedaddons;
...
function nextTurn() {
    ...
    text.innerText += "<br>.<br> Turn: " + turnTicker
}
